...
func (s Stack) PushList(letters []rune) Stack {
	return append(s, letters...)
}
...
func (s Stack) PopMultiple(size int) (Stack, []rune) {
	l := len(s)
	return s[:l-size], s[l-size:]
}
...
func main() {
	...
	// 	3: []rune{'P'},
	// }
	for i := instructionStart; i < len(items); i++ {
		ops, from, to := ReadInstruction(items[i])
		log.Printf("ops:%v from:%v to:%v", ops, from, to)

		stack, v := crates[from].PopMultiple(ops)
		log.Printf("Moving: %c", v)
		crates[from] = stack
		s2 := crates[to].PushList(v)
		log.Printf("Now: %c", s2)
